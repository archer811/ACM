...
struct array
{
    int m,start_time,end_time,s,index;
    friend bool operator < (array x,array y)
    {
        if(x.end_time==y.end_time)return x.index<y.index;
...
void insert(int index,int t,int m,int p,int mm)
{
    while(!myset.empty())
    {
        ...
        if(it->end_time<t)
        {
            ...
            {
                int u = q.front();
//                cout<<u<<"++++++++++++"<<nextime<< " "<<it->end_time<<endl;
                int num=0;
                int M=-1;
                int gg = ans[u].m;
                bool flag = false;
...
//                    cout<<"**"<<tmp.index<<" "<<tmp.start_time<<" "<<
//                        tmp.end_time<<" "<<M<<" "<<M+ans[u].m-1<<endl;
                    myset.insert(tmp);
    ...
    if(flag)
    {
        ...
        tmp.index = index;
        for(int i=M; i<M+m; i++)
            mark[i]=index,pre[i]=t+p-1;
        myset.insert(tmp);
//        cout<<index<<"  &  "<<tmp.start_time<< " "<<tmp.end_time<< " "<<tmp.s<<" "<<M+m-1<<endl;
    }
